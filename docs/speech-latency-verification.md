...
The task-owned primary drop-in was disabled, and the reviewed `235354c` entry release returned to **CPU-only**. Final CPU GUI first-play check: **1,952 ms**; live settings verified original endpoint and `maxRetries=0`. GPU service/runtime/tunnel configuration is retained as a prepared candidate, not an active primary. The task-created jump account was removed during cleanup (it was governed by Tailscale SSH rather than the intended OpenSSH key restrictions); re-enabling the tunnel requires provisioning/reviewing that jump identity, not merely starting the saved unit. Host retries remain zero. The original intermittent long-delay symptom is **not proven eliminated**. Improving the existing indirect network path is a remaining prerequisite to a defensible GPU-primary rollout, not authorization here to alter VPN/firewall topology.
